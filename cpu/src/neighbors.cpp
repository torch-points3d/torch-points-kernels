...
int nanoflann_neighbors(vector<scalar_t>& queries, vector<scalar_t>& supports,
                        vector<long>& neighbors_indices, vector<float>& dists,
  ...
  typedef nanoflann::KDTreeSingleIndexAdaptor<
      nanoflann::L2_Simple_Adaptor<scalar_t, PointCloud<scalar_t> >,
  ...
  std::vector<std::vector<std::pair<size_t, scalar_t> > > list_matches(
      pcd_query.pts.size());
...
int batch_nanoflann_neighbors(vector<scalar_t>& queries,
                              ...
                              vector<float>& dists, float radius, int max_num,
                              int mode) {
  // Initiate variables
  // ******************
  // indices
  ...
  for (auto& p0 : query_pcd.pts) {
    ...
    size_t nMatches =
        index->radiusSearch(query_pt, r2, all_inds_dists[i0], search_params);
    // Update max count

    if (nMatches > (size_t)max_count) max_count = nMatches;
    // Increment query idx
    i0++;
  }
      ...
      for (int j = 0; j < max_count; j++) {
        if ((unsigned int)j < inds_dists.size()) {
          neighbors_indices[i0 * max_count + j] =
              inds_dists[j].first + s_batches[b];
