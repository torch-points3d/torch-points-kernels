...
std::pair<at::Tensor, at::Tensor> ball_query(at::Tensor support,
  ...
  AT_DISPATCH_ALL_TYPES(query.scalar_type(), "radius_search", [&] {
    ...
    std::vector<scalar_t> supports_stl = std::vector<scalar_t>(
        data_s, data_s + support.size(0) * support.size(1));

    max_count = nanoflann_neighbors<scalar_t>(
        queries_stl, supports_stl, neighbors_indices, neighbors_dists, radius,
        max_num, mode);
  });
  auto neighbors_dists_ptr = neighbors_dists.data();
  long* neighbors_indices_ptr = neighbors_indices.data();
  if (mode == 0) {
    out = torch::from_blob(neighbors_indices_ptr, {query.size(0), max_count},
                           options = options);
    out_dists =
        torch::from_blob(neighbors_dists_ptr, {query.size(0), max_count},
                         options = options_dist);

  } else if (mode == 1) {
    out = torch::from_blob(neighbors_indices_ptr,
                           {(int)neighbors_indices.size() / 2, 2},
                           options = options);
...
std::pair<at::Tensor, at::Tensor> batch_ball_query(
    at::Tensor support, at::Tensor query, at::Tensor support_batch,
    at::Tensor query_batch, float radius, int max_num, int mode) {
  at::Tensor idx;
  ...
  query_batch =
      at::cat({at::zeros(1, query_batch.options()), query_batch.cumsum(0)}, 0);
  support_batch = degree(support_batch, batch_size);
  support_batch = at::cat(
      {at::zeros(1, support_batch.options()), support_batch.cumsum(0)}, 0);
  std::vector<long> query_batch_stl(
      query_batch.DATA_PTR<long>(),
      query_batch.DATA_PTR<long>() + query_batch.numel());
  std::vector<long> support_batch_stl(
      support_batch.DATA_PTR<long>(),
      support_batch.DATA_PTR<long>() + support_batch.numel());

  AT_DISPATCH_ALL_TYPES(query.scalar_type(), "batch_radius_search", [&] {
    std::vector<scalar_t> queries_stl(
        query.DATA_PTR<scalar_t>(), query.DATA_PTR<scalar_t>() + query.numel());
    std::vector<scalar_t> supports_stl(
        support.DATA_PTR<scalar_t>(),
        support.DATA_PTR<scalar_t>() + support.numel());

    max_count = batch_nanoflann_neighbors<scalar_t>(
        queries_stl, supports_stl, query_batch_stl, support_batch_stl,
        neighbors_indices, neighbors_dists, radius, max_num, mode);
  });
  auto neighbors_dists_ptr = neighbors_dists.data();
  long* neighbors_indices_ptr = neighbors_indices.data();
  if (mode == 0) {
    idx = torch::from_blob(neighbors_indices_ptr, {query.size(0), max_count},
                           options = options);
    dist = torch::from_blob(neighbors_dists_ptr, {query.size(0), max_count},
                            options = options_dist);
  } else if (mode == 1) {
    idx = torch::from_blob(neighbors_indices_ptr,
                           {(int)neighbors_indices.size() / 2, 2},
                           options = options);
